...
//   `https://recaptchaenterprise.googleapis.com/v1/projects/${process.env.NEXT_PUBLIC_GOOGLE_PROJECT_ID}/assessments?key=${process.env.NEXT_PUBLIC_GOOGLE_RECAPTCHA_API_KEY}`
//
...
//       "token": token,
//           "siteKey": secretKey,
//           "expectedAction": "enquiryFormSubmit"
//       }
//     },{
//     headers: {
...
const verifyRecaptcha = async (token) => {
    const verificationUrl = `https://www.google.com/recaptcha/api/siteverify?secret=${process.env.NEXT_PUBLIC_RECAPTCHA_SECRET_KEY}&response=${token}`
...
export default async function handler(req, res) {
    try {
        ...
        if (
            ...
            axios.post("https://getform.io/f/c9a0111b-5cf1-4a2a-a27d-fb7beb55420f",
                {
                    data: {email: email, subject: subject, message: message},
                })
                .then(res => console.log(res))
